...
```yaml
# tb project config
# Generated by `tb init`; rerun `tb init` to refresh with .bak backups.

# Board directory relative to this file.
board: .claude/board

# Task ID prefix.
prefix: WS

# Warn when starting a task above this many in-progress tasks.
wip_limit: 2

# File extensions scanned by `tb scan`.
...
Running `tb init` again in an existing project reconciles generated project files without touching existing tasks. It refreshes generated board docs and expands `.tb.yaml` with the supported config keys, keeping unset optional keys commented with defaults. Previous versions are saved as `.bak` files before overwrite. Explicitly provided flags override existing values; omitted flags keep their current values.
