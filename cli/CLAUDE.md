...
- `list.go` — filtering/sorting (priority rank then numeric ID), tabwriter output. Filter flags `-T`/`-p`/`-m`/`-s`/`-t`/`--parent` accept comma-separated values for OR-within-field matching; `--agent claude|codex|none` (multi-value, `none` matches unassigned) and `--search <term>` (case-insensitive substring against id and title) round out the GUI FilterBar parity. Filters AND across flags, OR within. Example: `tb ls -T bug,improvement -p P0,P1 --status ready --json`.
