...
func cmdGrep(args []string) {
	...
	statusFilter := fs.String("status", "all", "status filter: backlog|in-progress|done|archive|active|all (default: all)")
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, "Usage: tb grep <pattern> [--status backlog|in-progress|done|archive|active|all] [-s] [-l]\n\n")
