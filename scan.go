...
// scanExtensions lists file extensions to scan for TODO comments.
...
func scanForTodos(searchRoot, projectRoot string) []todoHit {
	...
	filepath.Walk(searchRoot, func(path string, info os.FileInfo, err error) error {
		...
		name := info.Name()
		if info.IsDir() {
			if strings.HasPrefix(name, ".") || name == "node_modules" || name == "vendor" || name == "build" || name == "dist" || name == "landing" || name == "tb" {
