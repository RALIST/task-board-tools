...
| `start` | `tb start 123` | Move task to in-progress |
| `done` | `tb done 123` | Move task to done |
| `mv` | `tb mv 123 done` | Move task to any status |
| `close` | `tb close 123` | Delete task from board |
...
--status status  Which directory: b, ip, r, d, or "all" (default: backlog)
...
```
--status status  Which directory: b, ip, r, d, or "all" (default: all)
...
Finds untagged `TODO`/`FIXME`/`HACK`/`WORKAROUND` comments in `.go`, `.ts`, `.svelte` files. Creates backlog tasks with type and module inferred from the comment and file path. Updates source comments in-place using the configured prefix:
...
BOARD.md includes an Epics section showing progress (done/total) for each epic.
...
```
project/
  .tb.yaml          Configuration (board path, prefix)
  board/             (or custom path)
    backlog/         Tasks ready to pick up
    in-progress/     Currently being worked on (max 2)
    done/            Completed
...
Each task is a markdown file (`PREFIX-NNN.md`) in a status directory. Moving a file between directories = status change. `BOARD.md` is regenerated automatically by `tb mv`/`start`/`done` and the stop hook.
