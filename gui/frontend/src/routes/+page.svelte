<script lang="ts">
  ...
  import { board, loaded, loadError, patchTask, refresh } from '$lib/stores/board';
  import { closeTask, openTask, selectedTaskId } from '$lib/stores/selection';
  import { dismissToast, pushToast, toasts } from '$lib/stores/toast';
  ...
  onMount(async () => {
    ...
    offEvents.push(
      Events.On('board:reloaded', async () => { await refresh(); }),
    );
    offEvents.push(
      Events.On('board:opened', async (info: any) => {
    ...
    // Single dispatcher for task:updated:* — the runtime gives us the event
    // name on the payload. We listen with a wildcard via the event name; if
    // the runtime doesn't dispatch by prefix we get a single full reload via
    // board:reloaded anyway, so missed task events degrade to a heavier
    // refresh rather than stale UI.
    offEvents.push(
      Events.On('task:updated', async (raw: any) => {
...
<main class="shell">
  ...
  {:else}
    <Board snapshot={$board} onSelect={openTask} />
...
<style>
  ...
  .err {
    ...
    color: var(--p0);
    font-size: 12px;
    border-top: 1px solid rgba(255, 90, 82, 0.3);
  }
  .hint {
    padding: 8px 18px;
    color: var(--fg-dim);
