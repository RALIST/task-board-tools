...
- [x] **F5.1** Daemon goroutine started in `main` and *activated* via `SettingsService.OpenBoard` hook (TB-53). Tasks with `AgentStatus=queued` and an `Agent` are picked up via (a) startup scan (TB-57) for the at-open-board case and (b) watcher emitter sink (TB-58) for live edits — including atomic CLI renames that route through `board:reloaded`, not `task:updated:<id>`. End-to-end smoke: `tb edit X -a claude --agent-status queued` from a terminal triggers a `running` transition within 5s.
- [x] **F5.2** Stale-running recovery: tasks with `AgentStatus=running` whose latest JSONL run has no `finished` event AND whose `started.pid` is dead (verified by `pidAlive(pid, expectedAgent)` TB-59, with node-wrapper fallback for npm-installed `claude`/`codex`) get a synthetic `finished{status: failed, reason: "stale after restart"}` appended and `AgentStatus` flipped to `failed` via `tb edit`. If `pidAlive==true`, the daemon leaves the task alone. **No re-attach in MVP** — F5.2's "re-attach" wording in `docs/FEATURES.md:179` is updated to match.
- [x] **F5.2 carve-out** A task whose latest JSONL event for the latest `run_id` is `finished{status: cancelled}` is reconciled to `AgentStatus=cancelled`, never `failed`. Defends the M4 5-step cancel ordering against a kill-9-mid-cancel race.
- [x] **F5.3** Worker pool: N workers reading a buffered task-ID channel, N = `MaxWorkers` (default 1, persisted at `preferences.json`, clamped to `[1, 4]`). In-memory active-set keyed by `task_id` plus `AgentService.HasActiveRun` cross-check prevents duplicate enqueue from racing `(startup scan, watcher event, manual UI run)`. Queue 3 tasks at once with default config → strictly sequential (asserted on JSONL timestamps).
- [x] **F5.4** Graceful shutdown: `Daemon.Close()` cancels the root ctx (which is now the parent of the runner ctx — see TB-54 ctx-plumbing prereq), waits up to 5s for workers to run the shared `finishCancelled(reason: "shutdown")` helper, then returns. Anything left behind is reconciled by next-start recovery.
- [x] **Structural** `AgentService.RunAgent` keeps its M4 contract for the drawer Run button (write queued + return run_id); daemon workers call a new internal `RunQueuedAgentSync(ctx, id)` that accepts queued state, plumbs ctx into the runner, and blocks until terminal. JSONL append / Wails emit / `tb edit` move outside `s.mu`; the mutex guards only `active` map insert/delete.
- [x] Single-instance lock (inherited from M2 / TB-2) keeps exactly one daemon per user — the second GUI invocation focuses the existing window without starting a second daemon.
- [x] `pid` and `run_id` are present in every `started` JSONL event (already done in M4 / TB-47); TB-54 also lands `agent` on `started` so TB-60's `pidAlive` source field is unambiguous.
- [x] All M5 sub-tasks (TB-53..TB-62) closed.
- [x] `docs/IMPLEMENTATION.md` M5 markers flipped to ☑; `docs/FEATURES.md` F5.2 wording updated (no re-attach); `docs/ARCHITECTURE.md` "Cancel carve-out" and "Daemon" sections updated to reference the M5 implementation (cf. TB-61 doc edits).
...
- 2026-05-14: Done — M5 shipped. All 10 children (TB-53..TB-62) closed. New `gui/internal/daemon` package: lifecycle `New`/`Activate`/`Deactivate`/`Close`, N-worker pool over buffered task-ID channel, in-memory active-set dedup cross-checked with `AgentService.HasActiveRun`, `pidAlive` two-step `comm`/`args` probe for npm shebang scripts, `EventSink` + `TeeEmitter` for watcher fan-out. New `gui/app/agent_recovery.go` implements `daemon.Recovery` (stale-running scan, JSONL replay, synthetic `finished{failed, "stale after restart"}`, cancelled carve-out). `gui/app/preferences.go` adds `$XDG_CONFIG_HOME/tb-gui/preferences.json` with `max_workers` (clamped [1,4]). TB-54 split public `RunAgent` from internal `RunQueuedAgentSync`: queued state synthesis for CLI-originated runs (when no JSONL queued event exists), narrowed `AgentService.mu` to active-map insert/delete with rollback on I/O failure, `agent` field added to `started` JSONL event, single `recordTerminal` helper gated by `ar.finishOnce` so `postRun`, `CancelRun`, and daemon-shutdown all produce exactly one terminal record per run. TB-62 daemon shutdown propagates ctx through `RunQueuedAgentSync`'s child ctx so `exec.CommandContext` cancel reaches in-flight agents; 5s WaitGroup grace then return. `SettingsService.OpenBoard` gained `BoardActivator` hook; `gui/main.go` wires daemon construction before `app.Run` + defers `daemon.Close()`. Tests: 50+ new Go tests pass with `-race`. F5.1 happy path (CLI-edit-then-pickup) and F5.4 shutdown-flushes-cancelled covered by real-`tb` integration tests; recovery covered by 5 Go in-process fixtures (no-finished+dead-PID, live-PID-skip, cancelled-carve-out, finished-md-stale, old-started-schema). Multi-process kill -9 scenarios documented in `gui/internal/daemon/README.md`. Docs: `docs/FEATURES.md` F5.1–F5.4 flipped to ☑ with no-re-attach wording; `docs/IMPLEMENTATION.md` M5 section flipped + entry in Completed work log.
- 2026-05-14: Done
