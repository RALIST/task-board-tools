...
func cmdList(args []string) {
	...
	statusFilter := fs.String("status", "backlog", "status directory (or 'all')")
	limit := fs.Int("n", 0, "limit results to N tasks (default: no limit)")

	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, "Usage: tb ls [-t tags] [-s size] [-m module] [-T type] [-p priority] [--parent ID] [--status backlog] [-n N]\n\n")
		fs.PrintDefaults()
	}
	...
	var dirs []string
	if strings.ToLower(*statusFilter) == "all" {
		dirs = statusDirs
	} else {
		resolved, err := resolveStatus(*statusFilter)
		if err != nil {
