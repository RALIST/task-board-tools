...
- Valid `AgentStatus` values: `queued`, `running`, `success`, `failed`, `cancelled`, `interrupted`, `lost`, `needs-user`. `failed` is reserved for an agent runner that actually reports a failed terminal outcome. `cancelled` is reserved for user-initiated cancellation. `interrupted` is reserved for recovery-initiated reconciliation when a captured session id makes the run resumable (TB-130). `lost` is recovery-initiated daemon/run-state loss: the task was `running`, but recovery cannot find a finished event or captured session, so it cannot honestly claim the agent failed. Resume eligibility is not derived from a single status string: the GUI offers Resume when the latest run has a captured session id and the task status is terminal (`interrupted`, `lost`, `failed`, `cancelled`, or `success`), while `queued`, `running`, and `needs-user` remain blocked. `needs-user` is the autonomous-agent handoff (TB-182): the agent stopped mid-run because a human decision is required; the task carries a `## User Attention` section with the specific ask. The daemon never writes `cancelled` from a crash or timeout, and convention reserves writes of `interrupted` and `lost` to `RecoverStale` even though the validator accepts them from any path (same precedent — invariant lives in code+docs, not the validator). Autonomous agents write `needs-user` through `tb edit --agent-status needs-user` together with `tb edit --user-attention -`; `recordTerminal` in the GUI runner has a carve-out that preserves `needs-user` over the exit-mapped status, so an agent that exits cleanly after asking still parks the task. Auto-groom and auto-implement skip `needs-user` tasks; manual Run/Groom in the GUI are blocked with an explanatory tooltip. The user resolves by editing the task body (or doing the action) and clearing the status with `tb edit <ID> --agent-status none`.
...
- **`session` event captured** → synthetic `finished{status:interrupted, reason:"interrupted by daemon restart"}`. The latest run also sets `AgentStatus:interrupted`; the GUI's Resume affordance is driven by the backend's captured-session flag plus terminal status, not by `interrupted` alone.
- **No `session` event** → synthetic `finished{status:lost, reason:"stale after restart"}`. The latest run also sets `AgentStatus:lost`. Resume isn't possible without an id, and `failed` is not used because recovery did not observe an agent-side failure.

If a `running` task has no JSONL at all, recovery synthesizes `finished{status:lost, reason:"running without JSONL"}` with a generated run id and sets `AgentStatus:lost`.

**Cancel carve-out**: recovery honors cancellation intent expressed in *either* the task's `.md` or the JSONL trail. If `AgentStatus` is already `cancelled` *or* the latest JSONL event for the latest `run_id` is `finished{status: cancelled}`, recovery reconciles to `cancelled` (writing `AgentStatus=cancelled` if the `.md` is out of sync) and never appends a `lost`/`interrupted` line. This defends the M4 5-step cancel ordering (kill → JSONL → Wails → `tb edit`) against a `kill -9` of the GUI between the JSONL write and the `tb edit`, and ensures a user-cancelled task with a captured session id still becomes `cancelled`, never `interrupted`. A cancelled/success/failed terminal run with a captured session remains resumable only as an explicit user action; the UI labels the source status near the Resume button.

Groom runs use the same JSONL/storage lifecycle with `mode:"groom"`. Review runs (TB-198) use `mode:"review"`. Resume runs use `mode:"resume"`. Each goes through its own decorator in `gui/internal/agent/runner.go` (`GroomingDecorator`, `ReviewDecorator`, `ResumeDecorator`) that overrides the prompt before delegating to the underlying Claude/Codex runner; the daemon and runGoroutine stay mode-agnostic. Review-mode agents write findings and move review state through the managed `tb review --pass` / `tb review --fail` surface and do not edit implementation files. Resume invokes `claude -r <uuid>` (same session id reused) or `codex exec --json resume <uuid> <prompt>` (a new id flows back through the translator's `OnSessionID` callback as a fresh `session` event — the new id is the resumable target for any future resume; the chain is traceable via each queued event's `resumed_from` + `resumed_from_run` fields).
...
The stages share the same agent runner, JSONL/log storage, cancellation, stale recovery, Wails event stream, and `max_workers` budget described above. Runs that start through `AgentService` directly reserve capacity against the same worker budget before launching, so manual runs, auto-groom, auto-implement, daemon-queued work, and resume/restart sweeps cannot exceed the configured concurrency together. They differ only in candidate selection, prompt decorator, and allowed kanban transition.

### Auto-groom

Auto-groom operates only on backlog tasks that `tb triage --json` still reports as needing grooming. It is off by default, requires a valid default agent, respects the configured worker budget, treats `wip_limit_ready` as a hard automation cap when configured, and uses an optional settle window so recently edited intake is not queued while a human is still shaping it. A groom-mode agent may refine goal, acceptance criteria, metadata, related tasks, or split work into subtasks. It does not own a status move. A successful groom run can be promoted from `backlog` to `ready` only through the managed `tb ready` gate, only when the task no longer appears in triage, and only while the ready-column WIP cap still has capacity. Manual `tb ready` keeps the configured board behavior: warn mode warns and allows, strict mode blocks.

### Auto-implement

Auto-implement operates only on committed `ready` tasks. It is off by default, requires a valid default agent and non-empty saved query, respects the configured worker budget, and applies the same filter language users see on the board. Candidate selection uses the task's assigned `Agent` when present and otherwise falls back to the default agent. Backlog tasks are never auto-implemented; they must first pass manual or automatic grooming into `ready`.
...
Auto-review operates only on `code-review` tasks. It is off by default via `auto_review_enabled=false` and requires a valid default agent (`claude` or `codex`) before the daemon may queue review work. The review target is the top-level `**ReviewRef:**` metadata; `## Review Target` is supplementary prose for humans. Missing `ReviewRef` writes a `needs-user` handoff and stops; it is not permission to guess from branch names, logs, or body text.

Review-mode agents are read-only against implementation files. On pass, they use `tb review --pass` to record "no blocking findings" and move the task to `done`. On fail, they use `tb review --fail`, which writes findings, returns the task to `ready`, adds the `review-failed` tag, and clears retry-blocking generic `AgentStatus` per TB-268 while preserving review attribution/history in per-mode fields and JSONL. A ready task tagged `review-failed` is input for auto-implement rework, not auto-review.

Auto-review dedupe and recovery are keyed to objective run state. Recovery applies only when the latest queued JSONL event for the task has `initiator=auto-review`: `interrupted` resumes the captured session, and `lost` starts a fresh review run. Manual review runs are never auto-resumed by this stage. A successful review-mode process that leaves the task in `code-review` without a managed pass/fail mutation is incomplete board state; the daemon must not infer pass or fail from free-form prose.
...
1. **On board activation**: stale-recovery scan → watcher event sink registered → startup grace → startup queue scan. The ordering is load-bearing: registering the sink before the scan closes the race where an edit lands between scan-read and subscription-attached. `automation_startup_grace_seconds` defaults to `30`, clamps to `[0,300]`, and `0` preserves immediate pickup. Grace delays only work-starting scans; stale recovery and deterministic reconciliation still run immediately, and watcher/settings changes during the grace window are coalesced into the scan at expiry.
...
4. **Worker pool**: idle worker goroutines read a buffered task-ID channel, but each run must acquire the live `MaxWorkers` concurrency gate before launching an agent. `MaxWorkers` defaults to 1, is configurable from 1 through the host CPU count via `$XDG_CONFIG_HOME/tb-gui/preferences.json`, and is applied at runtime when Settings changes; shrinking the budget does not cancel already-running work, but new work waits until capacity is available. In-memory active-set keyed by `task_id` plus `AgentService.HasActiveRun` cross-check prevents duplicate enqueue. Direct `AgentService` starts use the daemon's reservation surface as the same shared capacity budget, so manual runs, auto-groom, auto-implement, and their resume/restart sweeps cannot exceed the user's configured worker count.
