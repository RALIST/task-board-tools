...
- **CLI surface**: `tb review --submit | --target | --notes | --findings | --pass | --fail`. `--submit` accepts in-progress (happy path) and ready/backlog-with-`review-failed` (resubmit after rework, tag cleared on move; backlog source remains accepted for legacy boards). `--pass` writes no-blocking findings and moves task to done; `--fail` writes findings, moves task back to ready, adds `review-failed`, regenerates `BOARD.md` — all atomically under `.board.lock`.
