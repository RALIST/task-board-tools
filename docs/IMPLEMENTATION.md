...
4. ☑ Active-set dedup (in-memory) cross-checked against `AgentService.HasActiveRun`; live-reloaded `max_workers` setting (1 through host CPU count) at `preferences.json`
...
3. ☑ `auto-review` (TB-262): `code-review` tasks with a concrete `ReviewRef` are queued in review mode with `initiator=auto-review`; missing `ReviewRef` writes `needs-user`; pass moves to `done` through `tb review --pass`; fail moves to `ready` with `review-failed` through `tb review --fail`.
...
- ☑ TB-266: daemon reconciliation is soft/deterministic housekeeping only. It may repair objective missed transitions through managed CLI operations, but it must not infer pass/fail from prose, hot-loop on WIP blockers, or override `needs-user`, `cancelled`, or unrelated `interrupted`/`lost`.
- ☐ TB-270: prompt cleanup owns remaining prompt drift in `groom.md` and `implement.md`; daemon tasks should not paper over prompt drift.

**Acceptance**: the product docs and board guidance describe `auto-groom: backlog -> ready`, `auto-implement: ready -> in-progress -> code-review`, and `auto-review: code-review -> done|ready`; each stage is independently toggleable; auto-review is default-off through `auto_review_enabled`, requires a valid default agent, writes `needs-user` for missing `ReviewRef`, and recovers only JSONL runs queued by `initiator=auto-review`; failed review returns to `ready` with `review-failed` and a cleared generic scheduling cursor; auto-implement never picks a later epic child while an earlier same-parent child is unfinished; daemon reconciliation is documented as deterministic repair, not semantic guessing.
...
- 2026-05-19: TB-194 shipped — code-review column workflow (epic with TB-195/196/197/198/199/200). `code-review/` is a first-class active board status; `tb review --submit / --target / --notes / --findings / --pass / --fail` are the managed surface. The current canonical loop is in-progress -> code-review -> done on pass, or code-review -> ready with `review-failed` on fail. Submit accepts in-progress work and ready `review-failed` rework, clearing the marker on move; the CLI's backlog `review-failed` submit path is legacy compatibility for old boards only, not the staged workflow's fail destination or auto-implement source. `--pass` moves a code-review task to `done` with no-blocking findings, and `--fail` moves it back to `ready` with findings plus the `review-failed` tag. Review-mode agents are read-only against implementation files and mutate only board review state through `tb review` commands. Verification for the original epic: `cd cli && go test ./...`, `cd gui && go test ./app/... ./internal/...`, `cd gui/frontend && npm run check`, `cd gui/frontend && npm test -- --run`.
...
- 2026-05-19: TB-130 shipped — agent session resume + interrupted-run recovery. 12 sub-tasks landed in build order:
  ...
  - **TB-140/TB-252** Frontend Resume affordances, interrupted pill, and `↻ r_xxxx` chip. Visibility is driven by backend `agentResumable` (latest run has a captured session and the task status is terminal), not a hardcoded `interrupted` string; `agent:run-queued` handler extracts `resumed_from` / `resumed_from_run` from Wails payload.
...
- 2026-05-21: TB-315 tightened worker-budget enforcement. Auto-groom fresh starts, auto-groom resume/restart sweeps, auto-implement resume/restart sweeps, and manual/direct AgentService runs now reserve against the shared daemon/AgentService active set before launching. `max_workers` changes are applied to the running daemon via a live concurrency gate; lowering the value leaves existing runs alone and makes new work wait, while raising it wakes automation scans.
- 2026-05-21: TB-301 added `automation_startup_grace_seconds` (default 30, range 0-300) so board activation runs stale recovery immediately but delays daemon queued-task pickup plus auto-groom/auto-implement activation scans. Watcher events during the grace window are coalesced into the delayed scan, and deactivation/board switch cancels pending old-board startup work.
