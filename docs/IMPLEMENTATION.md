...
- 2026-05-19: TB-130 shipped — agent session resume + interrupted-run recovery. 12 sub-tasks landed in build order:
  ...
  - **TB-137** Recovery two-branch split: dead PID + SessionID → `markInterrupted("interrupted by daemon restart")` (NEW), no SessionID → existing `markFailed("stale after restart")`. Cancelled carve-out at lines 118-125 short-circuits first — user-cancelled with SessionID stays `cancelled`, never `interrupted`.
  ...
  - **TB-141** Fake-runner integration suite: `TestResumeCycle_KillRecoverResume` drives queued+started+session → RecoverStale → ResumeAgent through the shared `stubRunner`. `TestResumeCycle_KillBeforeSessionStaysFailed` locks the negative gate. Plus `TestResumeAgent_CodexHappyPath` mirror of the Claude happy path.
