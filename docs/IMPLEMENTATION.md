...
- 2026-05-19: TB-194 shipped — code-review column workflow (epic with TB-195/196/197/198/199/200). `code-review/` is a first-class active board status; `tb review --submit / --target / --notes / --findings / --pass / --fail` are the managed surface. Submit accepts in-progress (happy path) and ready/backlog-with-`review-failed` (resubmit-after-rework, clears the tag on move); `--pass` moves a code-review task to `done` with no-blocking findings, and `--fail` moves it back to `ready` with findings plus the `review-failed` tag. The historical backlog failure route was superseded by M10/F1.7. Review-mode agents are read-only against implementation files and mutate only board review state through `tb review` commands. Verification for the original epic: `cd cli && go test ./...`, `cd gui && go test ./app/... ./internal/...`, `cd gui/frontend && npm run check`, `cd gui/frontend && npm test -- --run`.
