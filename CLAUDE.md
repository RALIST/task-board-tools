...
- **`AgentStatus` values**: `queued | running | success | failed | cancelled | interrupted`. `cancelled` is user-initiated and `interrupted` is recovery-initiated (TB-130) — stale-recovery never overwrites `cancelled`, and convention reserves writes of `interrupted` to `RecoverStale` even though the validator accepts it from any path.
